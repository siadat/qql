...
func TestParseSQLErrors(t *testing.T) {
	cases := []struct {
		...
		{"in missing lparen", "WHERE x IN 1"},
		{"in empty list", "WHERE x IN ()"},
		{"in missing rparen", "WHERE x IN (1, 2"},
		{"in trailing comma", "WHERE x IN (1, 2,)"},
		{"in non-operand element", "WHERE x IN (1, =)"},
