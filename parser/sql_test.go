...
func TestParseSQL(t *testing.T) {
	cases := []struct {
		...
		{"limit before with", "SELECT * LIMIT 7 WITH prefix = '*.servers.*'", nil, "", false, nil, 7, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"full query with limit", "SELECT key, cpu WHERE cpu >= 8 ORDER BY cpu DESC LIMIT 2 WITH prefix = '*.servers.*'", []string{"key", "cpu"}, "", true, []OrderTerm{{Col: "cpu", Desc: true}}, 2, 0, WithOptions{Prefix: "*.servers.*"}, "cpu >= 8"},
		...
		{"offset before with", "SELECT * OFFSET 4 WITH prefix = '*'", nil, "", false, nil, -1, 4, WithOptions{Prefix: "*"}, ""},
		{"full query with offset", "SELECT key, cpu WHERE cpu >= 8 ORDER BY cpu DESC LIMIT 2 OFFSET 1 WITH prefix = '*.servers.*'", []string{"key", "cpu"}, "", true, []OrderTerm{{Col: "cpu", Desc: true}}, 2, 1, WithOptions{Prefix: "*.servers.*"}, "cpu >= 8"},

		{"with prefix star", "WITH prefix = '*'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*"}, ""},
		{"with prefix nested", "WITH prefix = '*.servers.*'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"with prefix literal", "WITH prefix = 'region-a.servers'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "region-a.servers"}, ""},
		{"with prefix lowercase", "with prefix = '*.servers.*'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"with prefix double quoted", `WITH prefix = "*.servers.*"`, nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"select then with", "SELECT key, cpu WITH prefix = '*.servers.*'", []string{"key", "cpu"}, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"star select then with", "SELECT * WITH prefix = '*.servers.*'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*"}, ""},
		{"full query with at end", "SELECT * FROM testdata/regions.yaml WHERE cpu >= 8 ORDER BY ram DESC WITH prefix = '*.servers.*'", nil, "testdata/regions.yaml", true, []OrderTerm{{Col: "ram", Desc: true}}, -1, 0, WithOptions{Prefix: "*.servers.*"}, "cpu >= 8"},
		...
		{"with provider then prefix", "WITH provider = 'external:./x.py', prefix = '*.servers.*'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*", Provider: "external:./x.py"}, ""},
		{"with prefix then provider", "WITH prefix = '*.servers.*', provider = 'external:./x.py'", nil, "", false, nil, -1, 0, WithOptions{Prefix: "*.servers.*", Provider: "external:./x.py"}, ""},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			cols, source, pred, orderBy, limit, offset, with, whereRaw, err := ParseSQL(c.src)
			if err != nil {
...
func TestParseSQLErrors(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			_, _, _, _, _, _, _, _, err := ParseSQL(c.src)
			if err == nil {
...
func TestParseSQLEvalIntegration(t *testing.T) {
	...
	_, _, pred, _, _, _, _, _, err := ParseSQL("SELECT age WHERE age >= 30")
	if err != nil {
